/** 可解析Lucene、json格式语法；依赖PEG.js:根据PEG文法生成parser
 * version:0.10.0;
 * web site:https://pegjs.org;
 **/
!(function(u){
    const PEG = u();
    let parser = null;
    let GrammarParser = function(options){
        if(!options)
            options = {
                type:'lucene'
            }
        ...
        if(PEG_GRAMMAR[this.type])
            parser = PEG.generate(PEG_GRAMMAR[this.type]);
        else
            throw '不支持"'+this.type+'"类型的解析！';
    ...
    GrammarParser.prototype = {
        parse(content,opt){
            ...
            try {
                result.notes = content.match(/\/\*(\n|.)*?\*\//g, '');
                content = $.trim(content.replace(/\/\*(\n|.)*?\*\//g, ''));
                let _po = parser.parse(content);
                this.check(_po);
                result.success = true;
                result.res = _po;
            }catch (e){
...
const TYPE_REG = {
    ...
    domain:/^(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})*(\.[a-zA-Z]{2,25})$/,
    hex:/^[0-9A-Fa-f]+$/
}
